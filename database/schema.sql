...
CREATE TABLE IF NOT EXISTS `utilizadores` (
    ...
    `perfil` ENUM('coordenador', 'chefe_departamento', 'gestor_academico', 'grh', 'presidente', 'secretario_geral', 'admin', 'docente') NOT NULL,
...
CREATE TABLE IF NOT EXISTS `planos_cobertura` (
    ...
    `estado` ENUM('Rascunho', 'Em Elaboração', 'Submetido', 'Aprovado pelo Departamento', 'Validado', 'Devolvido') DEFAULT 'Rascunho',
    `criado_por` INT DEFAULT NULL,
    `data_submissao` DATETIME DEFAULT NULL,
    `chefe_depto_id` INT DEFAULT NULL,
    `data_aprovacao_depto` DATETIME DEFAULT NULL,
    `parecer_depto` TEXT DEFAULT NULL,
    `presidente_id` INT DEFAULT NULL,
    `data_validacao_pr` DATETIME DEFAULT NULL,
    `parecer_pr` TEXT DEFAULT NULL,
    ...
    FOREIGN KEY (`chefe_depto_id`) REFERENCES `utilizadores`(`id`) ON DELETE SET NULL,
    FOREIGN KEY (`presidente_id`) REFERENCES `utilizadores`(`id`) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS `linhas_cobertura` (
    ...
    `decisao_aprovacao` VARCHAR(100) DEFAULT 'Aprovar',
    `observacoes` TEXT DEFAULT NULL,
    `updated_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
